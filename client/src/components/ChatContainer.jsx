...
const ChatContainer = () => {
  ...
  const { authUser, onlineUsers } = useContext(AuthContext)
    ...
    {/*for scrolling automatically when new msg comes */}
    useEffect(()=>{
    ...
    //Show messages upon selecting user
    useEffect(()=>{
      if (selectedUser){
  ...
  return selectedUser ? (
    <div className="h-full overflow-scroll relative backdrop-blur-lg">
      {/*make the header for selected user(user image, name, and info icon) */}
      ...
      {/*make inner chat content */}
      <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
        {/*send image, otherwise send text */}
        {/*css style justify-end for messages to be on the right and flex-row-reverse to be on left(incoming/outgoing msgs)*/}
      ...
      {/*make bottom texting input space */}
    ...
    <div className='flex flex-col items-center justify-center gap-2 text-gray-500 by-white/10 max-md:hidden'>
        {/*display logo and text without selected user) */}
